...
public class ServiceFilterModel : PageModel
{
    ...
    public Dictionary<int, string> DictServiceDelivery { get; set; } = new();

    public ServiceFilterModel(ILocalOfferClientService localOfferClientService, IPostcodeLookup postcodeLookup)
	{
                                 ...
                                 string searchText)
    {
        ...
        InitializeAgeRange();
        InitializeLanguages();
        SearchResults = await _localOfferClientService.GetLocalOffers("Family Experience",
                                                                      /*"Information Sharing",*/
                                                                      "active",
                                                                      null,
                                                                      null,
    ...
    private void CreateServiceDeliveryDictionary()
{
    ...
    private async Task GetLocationDetails(string postCode)
    {
        ...
        var (_, postcodeDetail) = await _postcodeLookup.Get(postCode);
            
        if (postcodeDetail != null)
        {
            CurrentLatitude = postcodeDetail.Latitude ?? 0.0d;
            CurrentLongitude = postcodeDetail.Longitude ?? 0.0d;
            DistrictCode = postcodeDetail.Codes.AdminDistrict;
            OutCode = postcodeDetail.Outcode;
        }
